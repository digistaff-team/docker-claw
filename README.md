...
- **Планировщик контента** — генерация и публикация постов (текст + изображение или видео) в Telegram-каналы по расписанию
- **Админ-панель** — отдельная зона `/admin` для управления всеми контейнерами: запуск, остановка, перезапуск, удаление, логи, прямое выполнение команд
- **Авторизация через Telegram** — центральный auth-бот (@clientzavod_bot) для входа новых пользователей по Telegram ID без пароля
...
┌──────────────────────────────────────────────────────────────┐
│                      Express Server                          │
│  /api/session  /api/execute  /api/files  /api/database       │
│  /api/manage   /api/plans    /api/apps   /api/content        │
│  /api/auth     /sandbox      /webhook    /hook/:id           │
│  /admin/*  (панель управления контейнерами)                  │
└──────┬──────────────────────┬───────────────────────────────-┘
       │                      │
┌──────▼──────────┐  ┌────────▼──────────┐
...
┌──────▼──────────────────────────────────────────────────┐
│                       manage/                           │
│  Auth Bot (@clientzavod_bot)  │  Telegram User Bots │
│  Email Poller (IMAP cron)        │  Hooks              │
│  Content Scheduler (публикации)  │                     │
└──────────────────────────────────┴─────────────────────┘
       │
┌──────▼─────────┐
...
По умолчанию используется образ `sandbox-python:latest` (задаётся через `.env`). Популярный вариант — официальный образ Microsoft Dev Containers:

```
mcr.microsoft.com/devcontainers/javascript-node:20-bookworm
```

Это образ на базе **Debian 12 (Bookworm)** с предустановленным **Node.js 20**. Образ скачается автоматически при первом старте сессии.
