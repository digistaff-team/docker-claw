/**
 * Репозиторий для vk_jobs и vk_publish_logs
 */
...
async function createJob(chatId, data) {
  return withClient(chatId, async (client) => {
    const result = await client.query(
      `INSERT INTO vk_jobs
        (chat_id, topic, group_id, post_text, hook_text, image_prompt, image_path, video_path, vk_content_type, link, status, image_attempts, correlation_id)
...
async function updateJob(chatId, jobId, data) {
  return withClient(chatId, async (client) => {
    ...
    const map = {
      topic: 'topic', groupId: 'group_id', postText: 'post_text',
      hookText: 'hook_text', imagePrompt: 'image_prompt',
      imagePath: 'image_path', videoPath: 'video_path',
      vkContentType: 'vk_content_type', link: 'link', status: 'status',
      errorText: 'error_text', imageAttempts: 'image_attempts',
      rejectedCount: 'rejected_count', vkPostId: 'vk_post_id',
    ...
    await client.query(
      `UPDATE vk_jobs SET ${fields.join(', ')} WHERE id = $${idx}`,
      values
    );
  });
}
...
async function getJobById(chatId, jobId) {
  return withClient(chatId, async (client) => {
    const result = await client.query(
      'SELECT * FROM vk_jobs WHERE id = $1',
      [jobId]
    );
...
async function listJobs(chatId, options = {}) {
  ...
  return withClient(chatId, async (client) => {
    ...
    const rows = await client.query(
      `SELECT * FROM vk_jobs ${where}
       ORDER BY created_at DESC
       LIMIT $${limitPos} OFFSET $${offsetPos}`,
      params
    );
    const totalRes = await client.query(
      `SELECT COUNT(*)::int AS total FROM vk_jobs ${where}`,
      status ? [status] : []
    );
    return {
...
async function addPublishLog(chatId, data) {
  return withClient(chatId, async (client) => {
    const result = await client.query(
      `INSERT INTO vk_publish_logs
        (job_id, group_id, vk_post_id, status, error_text, correlation_id)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id`,
      [
        data.jobId || null,
...
async function getJobsByStatus(chatId, status, limit = 10) {
  return withClient(chatId, async (client) => {
    const result = await client.query(
      `SELECT * FROM vk_jobs WHERE status = $1 ORDER BY updated_at ASC LIMIT $2`,
      [status, limit]
    );
    return result.rows;
  });
